/*
 *  Class: GameObj.java
 *  Version: 1.0
 *  Author: David Helminiak
 *  Date Created: 8 March 2018
 *  Date Last Modified: 9 March 2019
 *  Purpose: Provide methods for the game objects
 */


import java.util.LinkedList;
import java.util.Random;
import javafx.geometry.Bounds;
import javafx.scene.Group;
import javafx.scene.shape.Polygon;

public class GameObj {
	String name; //Name of the object
	int sizeOption = 0; //Current size option; enemy 1
	int size; //Current size; enemy 1
	int xSize, ySize; //Width and height of the object as used for window conditions; NOT ACCURATE FOR COLLISIONS
    int maxVelocity = 500; //Maximum velocity that the object can move
    int maxWeapons = 6; //Maximum number of fired weapons allowed on screen
    int weaponType = 1; //What type of weapon is currently engaged
	int j = 0; //Counter for polyArray formation
	int numVertices; //Number of vertices an object should have
	double timeToFire; //How often should this object fire a weapon; never unless indicated otherwise
	int numWeapons = 0; //Number of weapons that have been generated and exist
	double x, y, velocity, winWidth, winHeight, angle;
	double xCoord[], yCoord[], polyCoord[]; //Arrays to hold the x and y coordinates for the generated polygon
	boolean velocityUpFlag = false; //Should the velocity be increasing
	boolean velocityDownFlag = false; //Should the velocity be decreasing
	boolean angleUpFlag = false; //Should the angle be increasing
	boolean angleDownFlag = false; //Should the angle be decreasing
	boolean offBottom = false; //Has the object gone off the bottom of the screen
	boolean fireFlag = false; //Is the object firing a weapon
	boolean angleAlterable = false; //Is the object allowed to alter its angle
	boolean speedAlterable = false; //Is the object allowed to alter its speed
	boolean collision = false; //Has the object been hit by another object or weapon
	boolean player = false; //Is this object the player's ship
	static boolean boundaries = true; //Are objects allowed to move outside their traditional boundaries
	Polygon objectDescription; //Polygon describing the object
	Bounds boundary; //Boundary conditions for the object
	Random rand = new Random(); //Random number generator
	
	public void update(Group primaryGroup, double winWidth, double winHeight, double frameRate) {
		
		if (speedAlterable) { //If the speed of the object is able to be altered
			if (velocityUpFlag) { //If the object should move up
				if (velocity < maxVelocity) { //If the current velocity is less than the maximum allowable
					velocity+=1; //Increase the current velocity
				}
			}
			if (velocityDownFlag) { //If the object should move down
				if (velocity > 0) { //If the velocity is greater than 0
					velocity-=1;  //Decrease the velocity
				}
			}
		}
		else if (name.equals("enemy4")) { //If the enemy is type 4
			if (x >= (winWidth-xSize)) {
				angleUpFlag = true;
				angleDownFlag = false;
			}
			else if(x <= 0) {
				angleUpFlag = false;
				angleDownFlag = true;
			}
			if (angleUpFlag) { //If the object should move to the left
				x-=velocity/frameRate; //Move the object left
			}			
			if (angleDownFlag) { //If the object should move to the right
				x+=velocity/frameRate; //Move the object right
			}	
			if (velocityUpFlag) { //If the object should move up
				y-=velocity/frameRate; //Move the object down
			}
			if (velocityDownFlag) { //If the object should move down
				y+=velocity/frameRate; //Move the object up
			}
			if ((y+ySize+100) >= winHeight) {
				velocityUpFlag = true;
				velocityDownFlag = false;
			}
			else if (y <= 0) {
				velocityUpFlag = false;
				velocityDownFlag = true;
			}			
		}
		else if (name.equals("enemy5")) { //If the enemy is type 5
			if (x < 5) {
				angleDownFlag = true;
				angleUpFlag = false;
			}
			else if (x > (winWidth - xSize - 5) ){
				angleDownFlag = false;
				angleUpFlag = true;
			}
			else {
				angleDownFlag = false;
				angleUpFlag = false;
			}
			if (angleUpFlag) { //If the object should move to the left
				x-=velocity/frameRate; //Move the object left
			}			
			if (angleDownFlag) { //If the object should move to the right
				x+=velocity/frameRate; //Move the object right
			}	
			if (velocityUpFlag) { //If the object should move up
				y-=velocity/frameRate; //Move the object down
			}
			if (velocityDownFlag) { //If the object should move down
				y+=velocity/frameRate; //Move the object up
			}
			if ((y+ySize+100) >= winHeight) {
				velocityUpFlag = true;
				velocityDownFlag = false;
			}
			else if (y <= 0) {
				velocityUpFlag = false;
				velocityDownFlag = true;
			}			
		}
		else if (name.equals("enemy1")) {
			x += (velocity/frameRate) * Math.cos(Math.toRadians(angle)); //Update the x coordinate based on the velocity and angle
			y -= (velocity/frameRate) * Math.sin(Math.toRadians(angle)); //Update the y coordinate based on the velocity and angle
			objectDescription.setRotate(-angle+90); //Rotate the object according to the current angle
		}
		else { //Speed of the object is not able to be altered
			if (velocityUpFlag) { //If the object should move up
				y-=velocity/frameRate; //Move the object up
			}
			if (velocityDownFlag) { //If the object should move down
				y+=velocity/frameRate; //Move the object down
			}				
			if (angleUpFlag) { //If the object should move to the left
				x-=velocity/frameRate; //Move the object left
			}			
			if (angleDownFlag) { //If the object should move to the right
				x+=velocity/frameRate; //Move the object right
			}

		}
		if (angleAlterable) { //If the angle of the object is able to be altered
			if (angleUpFlag) { //If the angle should be increased
				angle+=1; //Increase the current angle
			}
			if (angleDownFlag) { //If the angle should be decreased
				angle-=1; //Decrease the current angle
			}
		}
		if (weaponType == 1) { //If the first weapon type is selected
			maxWeapons = 6;
		}
		else if (weaponType == 2) { //If the first weapon type is selected
			maxWeapons = 6;
		}
		else if (weaponType == 3) { //If the first weapon type is selected
			maxWeapons = 3;
		}		
		else if (weaponType == 4) { //If the first weapon type is selected
			maxWeapons = 2;
		}
		
		if (fireFlag) { //If the object is firing a weapon
			if (numWeapons < maxWeapons) { //If the number of weapons on screen is less than the maximum allowed

        		if (weaponType == 1) { //If the first weapon type is selected
        			primary.weapon1Sound.play();
        			
    				if (player) { //If this is the player object
    					primary.weapons.add(new Weapon1(primaryGroup, (x + xSize/2)-10, y, angle, velocity, winWidth, winHeight, player)); //Create a new weapon
            			primary.weapons.add(new Weapon1(primaryGroup, (x + xSize/2)+10, y, angle, velocity, winWidth, winHeight, player)); //Create a new weapon
    					numWeapons+=2; //Increase the count of how many weapons have been fired
    				}
    				else if (name.equals("enemy2")) { //If this is enemy 2
    					primary.weapons.add(new Weapon1(primaryGroup, (x + xSize/2)-10, y+ySize, angle, velocity, winWidth, winHeight, player)); //Create a new weapon
            			primary.weapons.add(new Weapon1(primaryGroup, (x + xSize/2)+10, y+ySize, angle, velocity, winWidth, winHeight, player)); //Create a new weapon
    				}
    				else if (name.equals("enemy3")) { //If this is enemy 3
    					primary.weapons.add(new Weapon1(primaryGroup, (x + xSize/2), y+ySize, angle, velocity, winWidth, winHeight, player)); //Create a new weapon
    				}
    				else if (name.equals("enemy4")) { //If this is enemy 4
    					primary.weapons.add(new Weapon1(primaryGroup, (x + xSize), y+(ySize/2), 0, velocity, winWidth, winHeight, player)); //Create a new weapon
    					primary.weapons.add(new Weapon1(primaryGroup, x, y+(ySize/2), 180, velocity, winWidth, winHeight, player)); //Create a new weapon

    				}
        		}
        		if (weaponType == 2) { //If the second weapon type is selected
        			primary.weapon2Sound.play();
        			if (player) { //If this is the player object
        				primary.weapons.add(new Weapon2(primaryGroup, (x + xSize/2)-10, y, 160, velocity, winWidth, winHeight, player)); //Create a new weapon
        				primary.weapons.add(new Weapon2(primaryGroup, (x + xSize/2)-10, y, 140, velocity, winWidth, winHeight, player)); //Create a new weapon
        				primary.weapons.add(new Weapon2(primaryGroup, (x + xSize/2)-10, y, 120, velocity, winWidth, winHeight, player)); //Create a new weapon
        				primary.weapons.add(new Weapon2(primaryGroup, (x + xSize/2)+10, y, 20, velocity, winWidth, winHeight, player)); //Create a new weapon
        				primary.weapons.add(new Weapon2(primaryGroup, (x + xSize/2)+10, y, 40, velocity, winWidth, winHeight, player)); //Create a new weapon
        				primary.weapons.add(new Weapon2(primaryGroup, (x + xSize/2)+10, y, 60, velocity, winWidth, winHeight, player)); //Create a new weapon
        				numWeapons+=6; //Increase the count of how many weapons have been fired
        			}
        			
        		}
        		if (weaponType == 3) { //If the third weapon type is selected
        			primary.weapon3Sound.play();
        			if (player) { //If this is the player object
        				primary.weapons.add(new Weapon3(primaryGroup, x+5, y-25, angle, velocity, winWidth, winHeight, player)); //Create a new weapon
        				numWeapons+=1;
        			}
        		}
        		if (weaponType == 4) { //If the fourth weapon type is selected
        			primary.weapon4Sound.play();
        			if (player) { //If this is the player object
        				if (primary.objects.size() <= 1) { //If there is nothing on screen
            				primary.weapons.add(new Weapon4(primaryGroup, (x + xSize/2), y-25, angle, velocity, winWidth, winHeight, player)); //Create a new weapon
            			}
            			else { //Find the closest object and set angle accordingly
            				int closestIndex = 1; //Best object found to aim for
            				double distance = 0; //distance measurement
                			double closestDistance = 10000; //Nearest distance found
                			double distX, distY; //x and y distance measurements
                			double destAngle; //Angle towards closest object
            				for (int i = 1; i < primary.objects.size(); i++) { //For all objects not including the player's ship
            					distX = Math.abs(x-primary.objects.get(i).x); //Delta x calculation
            					distY = Math.abs(y-primary.objects.get(i).y); //Delta y calculation
            					distance = Math.sqrt(Math.pow(distX, 2) + Math.pow(distY, 2)); //Calculate the actual distance
            					if (distance <= closestDistance) { //If the distance calculated is less then the current closest
            						closestDistance = distance; //Store the newly found closest distance
            						closestIndex = i; //Store the best object index to fire towards
            					}
            				}
            				destAngle = 180 + Math.atan2(primary.objects.get(closestIndex).y - y, x - primary.objects.get(closestIndex).x) * (180 / Math.PI); //Calculate angle towards nearest target
            				primary.weapons.add(new Weapon4(primaryGroup, (x + xSize/2), y-25, destAngle, velocity, winWidth, winHeight, player)); //Create a new weapon

            			}
        				numWeapons+=1;
        			}
        			else { //This is the enemy's object //Find the player's ship and set angle
        				double destAngle = 180 + Math.atan2(primary.objects.getFirst().y - y, x - primary.objects.getFirst().x) * (180 / Math.PI); //Calculate angle towards player's ship
            			primary.weapons.add(new Weapon4(primaryGroup, (x + xSize), y+(ySize/2), destAngle, velocity, winWidth, winHeight, player)); //Create a new weapon
            		}
        		}
        	}
			fireFlag=false; //Reset the indicator flag for whether the object is firing a weapon
		}

		//Update velocity impact on position
		if (speedAlterable) { //If the speed is allowed to be variable
			x += (velocity/frameRate) * Math.cos(Math.toRadians(angle)); //Update the x coordinate based on the velocity and angle
			y -= (velocity/frameRate) * Math.sin(Math.toRadians(angle)); //Update the y coordinate based on the velocity and angle
		}
		
		//Set window boundary conditions
		if (boundaries) { //If boundaries are being enforced
			if (name == "ship") { //If the object is a ship
				if (x >= winWidth-xSize) { //If the center of the object goes past the right of the screen
					x = winWidth-xSize; //Keep it on the right hand side of the screen
				}
				if (x <= 0) { //If the center of the object goes past the left of the screen
					x = 0; //Keep it on the left hand side of the screen
				}
				if (y >= winHeight-ySize) { //If the center of the object goes past the bottom of the screen
					y = winHeight-ySize; //Prevent the ship from going further down
				}
				if (y <= 0) { //If the center of the object goes past the top of the screen
					y = 0; //Prevent the ship from going further up
				}
			}
			else { //For all other objects
				if (!name.equals("enemy5")) { //Unless the object is an enemy of type 5
					if (x >= winWidth) { //If the center of the object goes past the right of the screen
						x = winWidth; //Keep it on the right hand side of the screen
					}
					if (x <= 0) { //If the center of the object goes past the left of the screen
						x = 0; //Keep it on the left hand side of the screen
					}
					else if (y >= winHeight) { //If the center of the object goes past the bottom of the screen
						offBottom = true; //Indicate that the object has gone past the bottom of the screen
					}
				}
			}
		}

		//Move the object
		if (angleAlterable) { //If the angle of the object is able to be altered
			objectDescription.setRotate(-angle+90); //Rotate the object according to the current angle
		}
		objectDescription.relocate(x,y); //Move the object to the appropriate x and y coordinates
		boundary = objectDescription.getBoundsInParent(); //Save the boundary conditions for collision detection

	}
		
	public void erase(Group primaryGroup) { //If the object needs to be erased
		primaryGroup.getChildren().remove(objectDescription); //Remove the object from the primary group	
	}

	public double newTimeToFire() {
		//Empty constructor; definition given within specific object classes
		return -1;
	}
    
}
