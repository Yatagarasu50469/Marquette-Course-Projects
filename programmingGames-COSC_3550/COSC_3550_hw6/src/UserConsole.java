...
public class UserConsole extends GameObj{
	...
	boolean w2Allowed = false; //Is the second weapon type allowed
	boolean w3Allowed = false; //Is the third weapon type allowed
	boolean w4Allowed = false; //Is the fourth weapon type allowed
	...
	int numOffset = 5; //Offset for numbers within weapon boxes
	Text t1 = new Text(); //New text node
	Text t2 = new Text(); //New text node
	Text t3 = new Text(); //New text node
	Text t4 = new Text(); //New text node
	Text t5 = new Text(); //New text node
	Text t6 = new Text(); //New text node
	Text t7 = new Text(); //New text node
	Text t8 = new Text(); //New text node	
	...
	Font headerFont, timeScoreFont;	
	UserConsole (Group primaryGroup, GraphicsContext gc, double winWidth, double winHeight) throws FileNotFoundException { //Constructor for a new user console
		this.winHeight = winHeight; //Store the current window height
		this.winWidth = winWidth; //Store the current window width
		headerFont = Font.loadFont(new FileInputStream(new File("data/PressStart2P.ttf")), 15);
		timeScoreFont = Font.loadFont(new FileInputStream(new File("data/PressStart2P.ttf")), 20);
		...
		t5.setFill(Color.BLACK); //Fill with black
		t5.setFont(timeScoreFont); //Set the font for the label
		t5.setLayoutX(x+numOffset); //Indicate where on the x layout the object is
		t5.setLayoutX(y+numOffset); //Indicate where on the y layout the object is
		t5.setText("1"); //Set the text
		consoleGroup.getChildren().add(t5); //Add the text node to the console group
		t5.relocate(x+numOffset, y+numOffset); //Move the object to the appropriate x and y coordinates		
		...
		t6.setFill(Color.BLACK); //Fill with black
		t6.setFont(timeScoreFont); //Set the font for the label
		t6.setLayoutX(x+numOffset); //Indicate where on the x layout the object is
		t6.setLayoutX(y+numOffset); //Indicate where on the y layout the object is
		t6.setText("2"); //Set the text
		consoleGroup.getChildren().add(t6); //Add the text node to the console group
		t6.relocate(x+numOffset, y+numOffset); //Move the object to the appropriate x and y coordinates	
		...
		t7.setFill(Color.BLACK); //Fill with black
		t7.setFont(timeScoreFont); //Set the font for the label
		t7.setLayoutX(x+numOffset); //Indicate where on the x layout the object is
		t7.setLayoutX(y+numOffset); //Indicate where on the y layout the object is
		t7.setText("3"); //Set the text
		consoleGroup.getChildren().add(t7); //Add the text node to the console group
		t7.relocate(x+numOffset, y+numOffset); //Move the object to the appropriate x and y coordinates	
		...
		t8.setFill(Color.BLACK); //Fill with black
		t8.setFont(timeScoreFont); //Set the font for the label
		t8.setLayoutX(x+numOffset); //Indicate where on the x layout the object is
		t8.setLayoutX(y+numOffset); //Indicate where on the y layout the object is
		t8.setText("4"); //Set the text
		consoleGroup.getChildren().add(t8); //Add the text node to the console group
		t8.relocate(x+numOffset, y+numOffset); //Move the object to the appropriate x and y coordinates	
		...
		timeRemaining.setLayoutY(y+15); //Set the text node's y layout position
		consoleGroup.getChildren().add(timeRemaining); //Add the text node to the console group
		timeRemaining.relocate(x, y+15); //Relocate the text node to the position on screen indicated
			
		x+=250; //Advance the x position
		...
		playerScore.setLayoutY(y+15); //Set the text node's y layout position
		consoleGroup.getChildren().add(playerScore); //Add the text node to the console group
		playerScore.relocate(x, y+15);	//Relocate the text node to the position on screen indicated
	...
	public void lostLife(int lives) { //Should the player have lost a life
		if (primary.lives == 3) { //If the number of lives remaining is 3
			el3.setOpacity(0); //Hide the third extra life indicator
		}
		if (primary.lives == 2) { //If the number of lives remaining is 2
			el3.setOpacity(0); //Hide the third extra life indicator
			el2.setOpacity(0); //Hide the second extra life indicator
		}
		if (primary.lives == 1) { //If the number of lives remaining is 1
			el3.setOpacity(0); //Hide the third extra life indicator
			el2.setOpacity(0); //Hide the second extra life indicator
	...
	public void allowWeapon(int allowedType) {
		if (allowedType == 2) {
			w2Allowed = true;
