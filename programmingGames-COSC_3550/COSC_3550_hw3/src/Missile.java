 ...
 *  Purpose: Provide methods for the player's weapon within the main program
 */
...
public class Missile {
	...
	int xSize = 8; //Width of the object
	int ySize = 10; //Height of the object
	boolean outsideWin = false; //Indicate whether the missile is outside of the window
	
	Missile(Group primaryGroup, double x, double y, double angle, double roombaVelocity, double winWidth, double winHeight) {
		this.x = x;
		this.y = y;
		this.angle = angle;
		this.velocity = roombaVelocity;
		velocity += 200; //Set the missile velocity as the roomba's + 200
		...
		objectDescription.setStrokeWidth(xSize);
