...
%crossProb = 60; %Chance out of 100 that the genes from the second random parent selected will be used in the offspring

function [functionValue, fitnessCount, finishtime] = Rosenbrock_sga_modified(maxGen, popSize, mutProb, crossProb);
...
crossArr = ones(1,100); %Array to hold mate identification for selection
mkdir (sprintf('%s_Test_%.0f', funcName, testNum)); %Make a folder to save generated images
for i=1:crossProb %For the cross probability given
    crossArr(i)=2; %Fill the crossArray as needed
end %end for
fitnessCount = 0; %How many times has fitness been evaluated
...
for i=1:1:popSize %For each of the individuals in the population
    fitnessCount = fitnessCount+1; %Increase counter
...
%while (genCount < maxGen) %While the current generation iteration is less than the maximum
	...
	if mod(popSize,2) == 0 %Check if the population size is even
		...
		while (j <= popSize/2) %For half of the population size
            ...
            %Prepare for crossover at the specified rate
            child1x = crossArr(round(rand()*99 + 1)); %Should child 1 gain x from mate 1 or mate 2 randomly
            child1y = crossArr(round(rand()*99 + 1)); %Should child 1 gain y from mate 1 or mate 2 randomly
            child2x = crossArr(round(rand()*99 + 1)); %Should child 2 gain x from mate 1 or mate 2 randomly
            child2y = crossArr(round(rand()*99 + 1)); %Should child 2 gain y from mate 1 or mate 2 randomly  
...
for i=1:1:popSize %For each of the individuals in the population
    ...
    %fprintf('The best fitness of: %e was found at coordinates x: %.0f y: %.0f \n',max(newPop(:,2)), x, y);
    %fprintf('The average fitness of the generation was %e \n',mean(newPop(:,2)));
    %fprintf('Fitness was evaluated: %.0f times\n',fitnessCount);
    %fprintf('There were %f percent of the population in the -%.0f <= x <= %.0f by -%.0f <= y <= %.0f area \n', ((inArea/popSize)*100),maxMinXYmesh,maxMinXYmesh,maxMinXYmesh,maxMinXYmesh);
    %fprintf('\n');
...
finishtime = toc(progStart)-totFigTime;
fprintf('The program finished running in: %f seconds. \n',finishtime') %Stop timer for the program run time
